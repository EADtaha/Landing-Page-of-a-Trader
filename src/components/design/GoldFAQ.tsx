...
const FAQS = [
  ...
  { q: 'What is your verified win rate?', a: "Our 12-month audited win rate is 81.4%, with an average risk-to-reward of 1:2.7. Full monthly breakdowns are shared publicly. Past performance does not guarantee future results." },
      ...
      <div className="max-w-3xl mx-auto">
        <div ref={ref} className="section-reveal text-center mb-16">
          <div className="inline-block text-xs font-semibold tracking-widest uppercase mb-4 px-4 py-1.5 rounded-full"
            style={{ color:'#C9A84C', border:'1px solid rgba(201,168,76,0.25)', background:'rgba(201,168,76,0.05)' }}>FAQ</div>
        ...
        <div className="space-y-3">
          {FAQS.map((faq, i) => (
            <div key={i} className="rounded-2xl overflow-hidden transition-all duration-200"
              style={{ border: open===i ? '1px solid rgba(201,168,76,0.35)' : '1px solid rgba(255,255,255,0.06)',
              ...
              <button className="w-full flex items-center justify-between px-6 py-5 text-left gap-4"
                ...
                <span className="font-semibold text-base" style={{ color: open===i ? '#E8C97A' : 'rgba(255,255,255,0.85)' }}>{faq.q}</span>
                <span className="text-lg flex-shrink-0 transition-transform duration-300"
                  style={{ color:'#C9A84C', transform: open===i ? 'rotate(45deg)' : 'rotate(0deg)' }}>+</span>
              </button>
